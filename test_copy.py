...
def outlook_propco_notification():
    subject = f'REQ{request_number} - PropCo Account Creation Notification - {user_first_name} {user_last_name}\n'
    body = f' \n Hello,\n\n Please create a PropCo Account for the following user:\n' \
...
####  TO DO -> Flow control - Automatic user find in Word Document.
...
print(clone_user)
###########################################################
user_job_title = automatic_input[14]
...
user_department = automatic_input[16]
...
if rps12_check_question in yes:
    ...
    if multiple_rps_database in yes:
        ...
        for i in range(number_of_rps_databases):
            ...
            reapit_extension = '@testExtraAlias.co.uk'
        for j in range(number_of_rps_databases):
            print(user_SAM_account + '-' + rps_database_string[j] + reapit_extension)
            setup_rps12_smtp = 'Set-ADUser ' + user_SAM_account + ' -add @{ProxyAddresses=''"smtp:' \
                               + user_SAM_account + '-' + rps_database_string[j] + reapit_extension + '"' + '}' + '\n'
            final_output.append(setup_rps12_smtp)
        print(f'{final_output}\n')



prop_check = str(input("PropCo account needed: "))

if prop_check in yes:
    outlook_propco_notification()
    print("Email Sent to Change Control")

vex_account = str(input("Vex account needed: "))

if vex_account in yes:
    print("VEX ACCOUNT HAS BEEN CREATED")
